...
async def release(update, context): await seller_action(update, context, "release")
async def refund(update, context): await seller_action(update, context, "refund")
async def cancel(update, context): await seller_action(update, context, "cancel")
...
async def buyer_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_mention = f"Admin ID {activator_id}" if activator_id else "Admin"
    ...
    await context.bot.send_message(
        ...
        text=f"📌 ADMIN APPROVAL NEEDED\n\n🆔 Deal: {deal_id(did)}\n👉 {admin_mention} please approve or cancel",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def admin_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if status == "ok":
        final = "REFUNDED" if action_type == "refund" else "CANCELLED" if action_type == "cancel" else "COMPLETED"
    else:
        final = "CANCELLED"
    ...
    if PROOF_CHANNEL:
        try:
            await context.bot.send_message(chat_id=str(PROOF_CHANNEL), text=text)
        except Exception as e:
            logger.error(e)
...
async def leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for admin, status in rows:
        stats.setdefault(admin, {"total":0,"completed":0,"refunded":0,"cancelled":0})
        stats[admin]["total"] += 1
    ...
    for admin, d in sorted(stats.items(), key=lambda x: x[1]["total"], reverse=True):
        text += (
            f"{rank}. @{admin}\n"
            f"📦 Total: {d['total']}\n"
