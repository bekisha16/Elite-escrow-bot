...
async def seller_action(update: Update, context: ContextTypes.DEFAULT_TYPE, action):
    ...
    await update.message.reply_text("Buyer approval needed", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def buyer_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buyer, action_type = cursor.fetchone()

    buyer = clean(buyer)
    user = clean(q.from_user.username)
    ...
    await q.edit_message_text("Waiting admin approval")

    await context.bot.send_message(
        q.message.chat_id,
        f"📌 Admin approval needed {deal_id(did)}",
...
async def admin_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    """, (
        final,
        q.from_user.id,
        q.from_user.username or "no_username",
        did
    ))
    ...
    await q.edit_message_text(
        ...
        f"💰 Amount: {amount}\n"
        f"💳 Method: {method}\n"
        f"👮 Handled by: @{q.from_user.username or 'no_username'}\n"
        f"📌 Status: {final}"
    )
...
async def leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("""
    SELECT handled_by_username, status
    FROM deals
    WHERE handled_by_username IS NOT NULL
    """)
    ...
    for admin, status in rows:
        ...
        name = f"@{admin}" if admin and admin != "no_username" else "Unknown"
    ...
    sorted_data = sorted(stats.items(), key=lambda x: x[1]["t"], reverse=True)
